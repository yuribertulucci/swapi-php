...
<div class="d-flex text-center flex-column justify-content-center mt-4">
    ...
    <form class="row w-100 justify-content-center" method="GET" action="{{ route('species.search') }}">
        <label for="search-input" class="visually-hidden">Search</label>
        <div class="input-group-append w-50 d-flex justify-content-center gap-2">
            <input type="text" id="search-input" name="query" placeholder="Search species..." class="form-control" value="{{ request()->getQuery('query', '') }}" />
...
<script>
    const apiUrl = '{{ route("api.species.index") }}' + '?page=' + '{{ request()->getQuery("page", 1) }}' + '&query=' + encodeURIComponent('{{ request()->getQuery("query", "") }}');
    $.ajax(apiUrl, {
        method: 'GET',
        dataType: 'json',
        async: true
